...
1. 纯文字通知用 text;富文本(链接、加粗)用 markdown。
2. 推图片:先用 upload-to-r2 上传得到 img.always1ov.com 链接,再用 image 模式推送。
3. 不必担心长度,脚本会自动分片;但内容尽量结构化(多用换行),分片会优先在换行处断开。
4. 失败排查:errcode 非 0 看返回 message;
   keywords not in content 表示没带关键词,检查 DINGTALK_KEYWORD;
   频繁发送报频控时,说明分片过多,可考虑精简内容。
