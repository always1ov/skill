...
def _reserve():
    """分片余量 = 关键词字节 + 编号标记(如 (12/34)\n) + 安全冗余。"""
    kw = os.environ.get("DINGTALK_KEYWORD", "").strip()
    return len(kw.encode("utf-8")) + 40
...
def send_text(text):
    # 不在分片前加关键词,改为每片各自补,避免后续片缺关键词被拒
    def mk(piece, i, total):
        body = piece if total == 1 else f"({i}/{total})\n{piece}"
        return {"msgtype": "text", "text": {"content": with_keyword(body)}}
    # reserve 给 (i/total) 标记 + 关键词 留余量
    _send_chunks(mk, text, reserve=_reserve())

def send_markdown(title, text):
    title = with_keyword(title)  # 标题每片都会带,关键词放标题即可满足校验
    def mk(piece, i, total):
        t = title if total == 1 else f"{title} ({i}/{total})"
        # markdown 校验同时看 title 和 text,标题已含关键词;text 兜底再补一次
        return {"msgtype": "markdown",
                "markdown": {"title": t, "text": with_keyword(piece)}}
    _send_chunks(mk, text, reserve=_reserve())
